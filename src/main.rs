...
fn run_puzzles(puzzles: Vec<Puzzle>, args: &Cli) {
    ...
    for p in puzzles {
        ...
        loop {
            ...
            if start.elapsed().as_secs() >= args.max_secs || runtimes.len() >= args.max_reps {
                break;
            }
        }
